...
class ControlLoop:
    ...
    def refresh_now(self) -> dict[str, Any]:
        with self._cycle_lock:
            ...
            tesla_snapshot = self.tesla_controller.peek_snapshot()
            if tesla_snapshot is not None:
                ...
                with self._cycle_lock:
                    ...
                    if automation_enabled:
                        ...
                        if tesla_snapshot is None:
                            ...
                            decision = {
                                "applied_amps": None,
                                "reason": "tesla_manual_refresh_required",
                                "command": None,
                                "command_result": None,
                            }
                            with self._lock:
                                ...
                                self._status.last_reason = "tesla_manual_refresh_required"
                                self._status.last_run_at = started_at.isoformat()
                                self._status.last_success_at = started_at.isoformat()
                                self._status.last_error = None
                                self._sync_pending_command_status_locked()
